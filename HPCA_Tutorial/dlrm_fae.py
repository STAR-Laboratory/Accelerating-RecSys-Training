...
if __name__ == "__main__":
	...
	forward_cold_time = 0
	backward_cold_time = 0
	optimizer_cold_time = 0
	...
	while k < args.nepochs:
		...
		for j, (X, lS_o, lS_i, T) in enumerate(train_normal_ld):
			...
			should_print = (itr_ctr % args.print_freq == 0)
			...
			forward_cold_time += end_forward - begin_forward
			backward_cold_time += end_backward - end_forward
			optimizer_cold_time += end_optimizing - end_backward
			total_forward_time += end_forward - begin_forward
			total_backward_time += end_backward - end_forward
			total_optimizer_time += end_optimizing - end_backward

			should_test = (
				(args.test_freq > 0)
				and (args.data_generation == "dataset")
				and (itr_ctr % args.test_freq == 0)
			)
				...
				if should_test and not args.inference_only:
					...
					for _, emb_dict in enumerate(hot_emb_dict):
						for _, (emb_no, emb_row) in enumerate(emb_dict):
							...
							data = torch.tensor(hot_emb[int(hot_row)])
							dlrm.emb_l[int(emb_no)].weight.data[int(emb_row)] = data
