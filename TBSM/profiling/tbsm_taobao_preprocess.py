...
if __name__ == "__main__":
	...
	parser.add_argument("--data-set", type=str, default="kaggle")  # or terabyte or alibaba
	...
	print("Profiling Threshold ", args.threshold)
	with torch.autograd.profiler.profile(args.enable_profiling, use_gpu) as prof:
		while k < args.nepochs:
			...
			if k == 0:
				...
				# =================== Filling Skew Table ======================
				for i, (lS_i, X, T) in enumerate(train_data):
				...
				for i in range(len(ln_emb)):
					...
					for j in range(ln_emb[i]):
						if((skew_table[i][j][0] * 100) / total_access[i] > args.threshold):
				...
				for i, train_tuple in enumerate(train_data):
					...
					if ( len(lS_i) == len(train_tuple[0])):
						train_hot.append((lS_i, train_tuple[1], train_tuple[2]))
					else:
						train_normal.append(train_tuple)
									

				profiling_end = time_wrap(use_gpu)
				#print("Profiling_Time ", 1000 * (profiling_end - profiling_begin))
